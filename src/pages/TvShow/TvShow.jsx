...
export const TvShow = ()=>{
        ...
        // Contexto 
        const { VITE_API , tvshow , setTvshow} = useContext(KudiContext)
        ...
        const getTvshow = async ()=>{
            ...
            await fetch(`${VITE_API}/tvshows/${_id}` , options) // Fetch al endpoint /movie para obtener los datos
            .then(res => res.json())
            .then( data => setTvshow(data))             // Setear datos de la respuesta a movies
    ...
    return(
        ...
        <article className="Article Movie">
            <div className="Article-wrapper">
                <h2 className="Article-h2 H2">{tvshow.title}</h2>
                <div className="Span-wrapper">
                    <span className="Article-span">{tvshow.pegi}</span>
                ...
                <ul className="Article-ul Genre-ul">
                    {!tvshow && !tvshow.genre && tvshow.genre.length == 0 && <li>-</li>}
                    {tvshow && tvshow.genre && tvshow.genre.length != 0 && tvshow.genre.map( (eachGenre , index) =>                 // Al ser array, se utiliza el index como key para iterar sobre los elementos del array
